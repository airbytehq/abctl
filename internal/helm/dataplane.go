...
func InstallDataplaneChart(ctx context.Context, client goHelm.Client, namespace, releaseName string, credentials *api.CreateDataplaneResponse, config *abctl.Config) error {
...
func buildDataplaneValues(credentials *api.CreateDataplaneResponse, config *abctl.Config) string {
...
metrics:
  enabled: "false"
`, config.AirbyteURL, config.AirbyteAPIHost, credentials.DataplaneID, credentials.ClientID, credentials.ClientSecret, config.AirbyteAuthURL, config.Edition)
}
