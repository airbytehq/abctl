...
func TestTextInputModel_Backspace(t *testing.T) {
	...
	model.value = "test" // Set value manually for backspace test

	// Test backspace
	result, _ := model.Update(tea.KeyMsg{Type: tea.KeyBackspace})
	model = result.(TextInputModel)
	if model.value != "tes" {
		t.Errorf("Expected value='tes' after backspace, got %s", model.value)
	}
	...
	model = result.(TextInputModel)
	if model.value != "" {
		t.Errorf("Expected value='' after backspace on empty, got %s", model.value)
	}
}
...
func TestTextInputModel_EmptyInputUsesPlaceholder(t *testing.T) {
	...
	model.value = "" // Clear the value to test placeholder usage
