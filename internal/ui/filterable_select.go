...
func (m *FilterableSelectModel) adjustViewport() {
...
func (m *FilterableSelectModel) filterOptions() {
	...
	if len(filtered) > 0 {
		m.selected = 0
		m.viewport = 0 // Reset viewport when filtering
...
func (m FilterableSelectModel) View() string {
	...
	// Show filtered options with scrolling
	if len(m.filtered) == 0 {
		s += "  No matches found\n"
	} else {
		const maxVisible = 10
		...
		// Show only the visible portion of filtered options
