...
func (m FilterableSelectModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		default:
			// Check if it's a printable character to start filtering
			if len(msg.String()) == 1 && msg.String() >= " " && msg.String() <= "~" {
				// Add character to filter
				currentValue := m.filterInput.Value()
				m.filterInput.SetValue(currentValue + msg.String())
				...
				if len(currentValue) > 0 {
					m.filterInput.SetValue(currentValue[:len(currentValue)-1])
					// Re-filter
					m.filterOptions()
				}
				return m, nil
